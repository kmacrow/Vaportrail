...
$service_id = $path_params[0]; //$_GET['oauth_service_id'];
$oauth_action = $path_params[1];
...
switch($oauth_action){
	...
	case 'complete':
		...
		exit('<html><head><script>window.opener["'.$service_id.'_oauth_complete"]();self.close()</script></head></html>');
	break;
	case 'api':
