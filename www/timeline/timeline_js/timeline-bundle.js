Timeline._Band = function(B, G, C) {
    ...
    this._labeller = "labeller" in G ? G.labeller : "createLabeller" in B.getUnit() ? B.getUnit().createLabeller(this._locale, this._timeZone) : new Timeline.GregorianDateLabeller(this._locale, this._timeZone, this);
    this._theme = G.theme;
...
Timeline._Band.prototype._moveEther = function(A) {
    ...
    if (this._viewOffset > -this._viewLength * .5 || this._viewOffset < -this._viewLength * (Timeline._Band.SCROLL_MULTIPLES - 1.5)) {
        this._recenterDiv();
    } else {
        this.softLayout();
    }
...
Timeline.GregorianDateLabeller = function(B, A, C) {
    this._locale = B;
    this._timeZone = A;
    this._band = C || {'_bandInfo':{'params':{}}}
};

Timeline.GregorianDateLabeller.monthNames = [];

Timeline.GregorianDateLabeller.dayNames = ['Mon','Tue','Wed','Thu','Fri','Sat','Sun'];
...
Timeline.GregorianDateLabeller.prototype.defaultLabelInterval = function(B, C) {
    ...
    switch (C) {
      ...
      case SimileAjax.DateTime.DAY:
      	    if(this._band._bandInfo.params['paintLabels'] == true)
        	D = Timeline.GregorianDateLabeller.dayNames[B.getDay()] + ", " + Timeline.GregorianDateLabeller.getMonthName(B.getUTCMonth(), this._locale) + " " + B.getUTCDate();
            else
                D = ''
        break;
      ...
      case SimileAjax.DateTime.MILLENNIUM:
        ...
        F = C == SimileAjax.DateTime.MONTH || C == SimileAjax.DateTime.DECADE && E % 100 == 0 || C == SimileAjax.DateTime.CENTURY && E % 1e3 == 0;
        break;
      default:
        D = B.toUTCString();
    }
    ...
    if (H && H.events) {
        ...
        for (var G = 0; G < H.events.length; G++) {
            ...
            var L = new Timeline.DefaultEventSource.Event({
                ...
                hash: "hash" in A ? A.hash: undefined,
                start: E(A.start),
                end: E(A.end),
...
Timeline.DefaultEventSource.Event = function(A) {
    ...
    var C = A.id ? A.id.trim() : (A.hash ? A.hash : "");
...
Timeline._Impl.prototype._initialize = function() {
    ...
    for (var B = 0; B < this._bandInfos.length; B++) {
        ...
        if("highlight" in F){
        	this._bands[B]._highlight = F.highlight
        }
    }
    if (this.autoWidth) {
